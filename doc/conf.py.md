# conf.py Customization

Sphinx uses a configuration file `conf.py` to control its operation. **rosdoc2** generates a default `conf.py` file, but then wraps that file with additional values to support its operation.

A user-defined `conf.py`, if desired, should by added to the user's documentation subdirectory (`doc` by default).

The default `conf.py` generated by **rosdoc2** has fairly good internal documentation of what is available. That default file can be seen in the build folder (`docs_build` by default) with the name `__conf_default.py` in a subdirctory `PACKAGE_NAME/PACKAGE_NAME/wrapped_sphinx_directory`. If you want to provide a custom `conf.py`, that is a good place to start. Take that file, modify it, and place it in your documentation folder.

An example `__conf_default.py` file, from the `minimal_package` test package, is:
```python
...
copyright = 'The <minimum_package> Contributors. License: Apache License 2.0'
author = """Some One"""
